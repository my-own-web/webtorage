  ...
  return (
    <BoxBlock>
      {site_name ? <div><Sitename>{`[${site_name}]`}</Sitename>
        <ChangeButton onClick={onRemove}>{"삭제하기"}</ChangeButton><ChangeButton onClick={() => { onFinishCategory(editCategory); onClickCategory() }}>{"카테고리 수정"}</ChangeButton>{onCategory(editCategory)}</div> : ''}
